...
class AddTeacherAttendence
{
	JFrame f;
	Choice cbRoll;
	JLabel lbl1,lbl2,lbl4;
	JComboBox cb, cbRollNo;
	JButton btnSubmit, btnCancel;

	AddTeacherAttendence()
	{
		f = new JFrame("Teacher Attendence");
		...
		try
		{
			...
			ResultSet rs = state.executeQuery("SELECT id FROM addteacher");

			while(rs.next())
			{
				cbRoll.add(rs.getString("id"));
			}
		}
		catch(Exception e)
		{
		...
		btnSubmit.addActionListener(new ActionListener()
			{
				...
				{
					String id = (String)cbRoll.getSelectedItem();
					String date = new Date().toString();
					String attendance = (String)cb.getSelectedItem();
					...
					try
					{
						...
						String query = "INSERT INTO teacherattendance VALUES('"+id+"', '"+date+"', '"+attendance+"')";

						state.executeUpdate(query);

						JOptionPane.showMessageDialog(null, "Successfully Insert The Attendance");
						f.setVisible(false);
